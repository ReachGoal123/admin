...
class InstallCommand extends Command
{
    ...
    {
        if ($this->confirm('Do you want publish the mrzeta/admin\'s migrations ?')) {
            $this->call('admin:migration');
        }
        ...
        if ($this->confirm('Do you want run the mrzeta/admin\'s seeders now ?')) {
            $this->call('admin:seed');
        }

        if ($this->confirm('Do you want publish configuration files from mrzeta/admin package ?')) {
            $this->call('vendor:publish', [
                '--provider' => 'Mrzeta\Admin\AdminServiceProvider',
                ['--tag' => ['config']],
            ]);
        }

        if ($this->confirm('Do you want publish assets from mrzeta/admin package ?')) {
            $this->call('vendor:publish', [
                '--provider' => 'Mrzeta\Admin\AdminServiceProvider',
                ['--tag' => ['assets']],
            ]);
